...
import { getSession } from "@/lib/session"; // <--- Import your existing function

export default async function HomePage() {
  // 1. Get the session using your existing library
  const session = await getSession();

  // 2. Security Check: If no session or no userId, kick them out
  if (!session || !session.userId) {
    redirect("/");
  }

  // 3. Use the real User ID (Type assertion needed because jose payload is generic)
  const userId = session.userId as string;

  // 4. Fetch Data with the real ID
  const userData = await getUserStats(userId);

  if (!userData) {
    // Edge case: User has a session but was deleted from DB
    redirect("/");
  }
      ...
      <main className="max-w-4xl mx-auto px-4 py-10 space-y-10">
        ...
        {userData.stats.posts === 0 ? (
          <div className="text-center py-20 bg-white/50 rounded-xl border border-dashed border-gray-300">
            <div className="w-16 h-16 bg-[#E9EEE8] rounded-full flex items-center justify-center mx-auto mb-4 text-[#1F4F46]">
              <PenSquare size={24} />
            </div>
